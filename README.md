...
[![Version](https://img.shields.io/badge/version-2025.11.08.18.22-blue.svg)](https://github.com/Git-think/Qwen2Api-Proxy)
...
为了方便在不重启整个应用或修改 `.env` 文件的情况下动态调整配置，本项目支持通过在 `data` 文件夹中创建特定文件来管理配置。应用在**启动时**会检测并处理这些文件。
...
1.  **修改服务端口:**
    - 在 `data` 文件夹中创建一个名为 `set-env` 的文件。
    - 在文件中写入一行：`SERVICE_PORT=3066`
    - 重启应用，服务将在 `3066` 端口启动。

2.  **动态添加一个新账户:**
    - 在 `data` 文件夹中创建一个名为 `add` 的文件。
    - 在文件中写入一行：`ACCOUNTS=new_user@example.com:new_password`
    - 重启应用，新账户将被添加到系统中。

3.  **从 `.env` 重新加载代理:**
    - 在 `.env` 文件中的 `SOCKS5_PROXIES` 添加了新的代理地址。
    - 在 `data` 文件夹中创建一个名为 `reload-env` 的文件。
    - 在文件中写入一行：`proxy`
    - 重启应用，新的代理将被加载，并且所有代理的状态会被重置为 `untested`。
